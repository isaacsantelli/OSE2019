...
def EV_F_ITER(X, k_init, n_agents, grid):
    ...
    for (z in range(5)):
        VT_sum += (1/5)*beta*grid[z].evaluate(knext)
...
def EV_GRAD_F(X, k_init, n_agents, z):
    ...
    for ixN in range(N):
        ...
        if (xAdj[ixN] - h >= 0):
            xAdj[ixN]=X[ixN] + h
            fx2=EV_F(xAdj, k_init, n_agents,z)

            xAdj[ixN]=X[ixN] - h
            fx1=EV_F(xAdj, k_init, n_agents, z)
...
def EV_G_ITER(X, k_init, n_agents, z):
    N=len(X)
    M=3*n_agents+1  # number of constraints
    G=np.empty(M, float)
...
def EV_JAC_G(X, flag, k_init, n_agents, z):
    N=len(X)
    M=3*n_agents+1
    NZ=M*N
    A=np.empty(NZ, float)
    ...
    if (flag):
        ...
        # Finite Differences
        h=1e-4
        gx1=EV_G(X, k_init, n_agents, z)

        for ixM in range(M):
            for ixN in range(N):
                xAdj=np.copy(X)
                xAdj[ixN]=xAdj[ixN]+h
                gx2=EV_G(xAdj, k_init, n_agents, z)
...
def EV_JAC_G_ITER(X, flag, k_init, n_agents, z):
    N=len(X)
    M=3*n_agents+1
    NZ=M*N
    A=np.empty(NZ, float)
    ...
    if (flag):
        ...
        # Finite Differences
        h=1e-4
        gx1=EV_G_ITER(X, k_init, n_agents, z)

        for ixM in range(M):
            for ixN in range(N):
                xAdj=np.copy(X)
                xAdj[ixN]=xAdj[ixN]+h
                gx2=EV_G_ITER(xAdj, k_init, n_agents, z)
                A[ixN + ixM*N]=(gx2[ixM] - gx1[ixM])/h
        return A
